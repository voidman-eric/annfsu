...
class UserCreate(BaseModel):
    ...
    address: str
    institution: str
    committee: str  # central, provincial, district, campus
...
class ContentCreate(BaseModel):
    type: str  # news, knowledge, constitution, oath, quotes, about
    title_ne: str
    content_ne: str
...
class SongCreate(BaseModel):
    title_ne: str
    category: str  # organizational, national_anthem
...
async def create_member(user_data: UserCreate, admin: dict = Depends(require_admin)):
    ...
    user_dict["status"] = "approved"  # Admin creates, so auto-approved
    user_dict["created_at"] = datetime.utcnow()
    user_dict["updated_at"] = datetime.utcnow()
    
    # Generate membership ID
    count = await db.users.count_documents({}) + 1
    user_dict["membership_id"] = f"ANNFSU-{count:05d}"
    user_dict["issue_date"] = datetime.utcnow().isoformat()
...
async def get_members(admin: dict = Depends(require_admin), status_filter: Optional[str] = None):
    query = {}
    if status_filter:
...
async def update_member(member_id: str, update_data: UserUpdate, admin: dict = Depends(require_admin)):
    user = await db.users.find_one({"_id": ObjectId(member_id)})
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Member not found")
...
async def get_content_by_type(content_type: str):
    contents = await db.content.find({"type": content_type}).to_list(1000)
    return [
        ContentResponse(
            id=str(content["_id"]),
            type=content["type"],
...
async def update_content(content_id: str, content_data: ContentCreate, admin: dict = Depends(require_admin)):
    content = await db.content.find_one({"_id": ObjectId(content_id)})
    if not content:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Content not found")
    
    update_dict = content_data.dict()
    update_dict["updated_at"] = datetime.utcnow()
...
async def update_contact(contact_id: str, contact_data: ContactCreate, admin: dict = Depends(require_admin)):
...
@api_router.get("/")
async def root():
    return {"message": "ANNFSU API - अखिल नेपाल राष्ट्रिय स्वतन्त्र विद्यार्थी युनियन"}
